...
const KurikulumPendaftaran = () => {
  ...
  return (
    <aside
      className={`fixed rounded-md right-0 top-24 h-[85vh] ${
        !sideActive ? "translate-x-full" : "translate-x-0 md:translate-x-6"
      } transition-transform duration-200 ease-in-out w-xs bg-green-700`}
    >
      <div
        onClick={sideBar}
        className={`flex justify-center items-center ${
          sideActive
            ? "size-12 -left-12"
            : "min-h-8 py-2 w-[7rem] md:w-44 px-2 md:px-4 -left-[7rem] md:-left-[10.5rem]"
      ...
      <div className="w-full h-full rounded-md overflow-y-scroll">
        ...
        <ul>
          {dataKurikulum.map((kelas) => {
            return (
              <li className="py-2 group cursor-pointer font-semibold text-green-700 border-b-4">
                <div
